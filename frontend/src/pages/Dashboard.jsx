...
function buildChartData(bookings) {
  ...
  return months.map(({ month, key }) => {
    ...
    return {
      month,
      bookings: mb.length,
      revenue:  mb.reduce((s, b) => s + (b.totalAmount || 0), 0) / 1000,
    };
  });
}
...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full
                      text-xs font-medium border ${cfg.bg} ${cfg.text}`}>
      ...
      {payload.map((p, i) => (
        <p key={i} className="flex items-center gap-2" style={{ color: p.color }}>
          ...
          {p.name}: <span className="font-bold ml-1">
            {p.name === 'revenue' ? `₦${p.value}k` : p.value}
          </span>
        </p>
      ))}
    </div>
...
export default function Dashboard() {
  ...
  // activeMode is the ONLY thing that drives "am I in owner or renter view"
  // It comes directly from AuthContext — no local role state, no user.role checks
  const { user, can, activeMode, canSwitchMode } = useAuth();
  ...
  // ── isOwner: the single derived boolean all JSX reads from ────────────────
  // Recomputes on every render when activeMode changes — no caching
  const isOwner = activeMode === 'owner' && can.list;

  // ── modeLabel: shown in header subtitle ───────────────────────────────────
  // Updates immediately on every re-render — no useState, no useEffect needed
  const modeLabel = can.admin ? 'Admin'
                  : isOwner   ? 'Owner Mode'
                  :             'Renter Mode';

  const modeDot = can.admin ? 'bg-red-500'
                : isOwner   ? 'bg-[#1a5c3a]'
                :              'bg-blue-500';
    ...
    try {
      ...
      if (isOwner) {
        ...
        setStats({
          ...
          earnings:      bookings.filter(b => b.paymentStatus === 'paid')
                                 .reduce((s, b) => s + b.totalAmount, 0),
          recentActivity: [...toolEvents, ...bookingEvents]
        ...
        setStats({
          ...
          approved:       bookings.filter(b => b.status === 'approved').length,
          completed:      bookings.filter(b => b.status === 'completed').length,
          spent:          bookings.filter(b => b.paymentStatus === 'paid')
                                  .reduce((s, b) => s + b.totalAmount, 0),
      ...
      lastFetchedMode.current = activeMode;
  ...
  // Refetch when mode switches — only if we already have data from a previous fetch
  useEffect(() => {
    if (!mountFetched.current) return;
    if (lastFetchedMode.current === null) return;
    if (lastFetchedMode.current === activeMode) return;
    fetchAll();
  }, [activeMode, fetchAll]);
  ...
  const statCards = isOwner ? [
    {
      label:     'My Tools',
      value:     loading ? '—' : stats?.totalTools ?? 0,
    ...
    },
    {
      ...
      value:     loading ? '—' : `₦${(stats?.earnings ?? 0).toLocaleString()}`,
      icon:      <TrendingUp size={17} />,
      iconBg:    'bg-[#d4eadd]',
      iconColor: 'text-[#154d30]',
      link:      '/booking-requests',
    },
  ] : [
    {
    ...
    {
      label:     'Total Spent',
      value:     loading ? '—' : `₦${(stats?.spent ?? 0).toLocaleString()}`,
      icon:      <TrendingUp size={17} />,
      iconBg:    'bg-[#eef6f1]',
      iconColor: 'text-[#1a5c3a]',
      link:      '/bookings',
  ...
  const quickActions = isOwner ? [
    {
      label: kycApproved ? 'List a new tool' : 'Complete KYC to list',
      to:    kycApproved ? '/tools/new' : '/kyc',
      icon:  '➕',
      desc:  kycApproved ? 'Add equipment to your listings' : 'Required before listing',
    },
    {
      label: 'My Tools',
      to:    '/my-tools',
      icon:  '🔧',
      desc:  `${stats?.totalTools ?? 0} total listings`,
    },
    {
      label: 'Booking Requests',
      to:    '/booking-requests',
      icon:  '📋',
      desc:  `${stats?.pending ?? 0} pending approval`,
    },
    {
      label: 'Payout Settings',
      to:    '/bank-details',
      icon:  '🏦',
      desc:  'Manage bank details',
    },
  ] : [
    {
      label: 'Browse Tools',
      to:    '/tools',
      icon:  '🔍',
      desc:  'Find equipment near you',
    },
    {
      label: 'My Bookings',
      to:    '/bookings',
      icon:  '📋',
      desc:  `${stats?.totalBookings ?? 0} total bookings`,
    },
    {
      label: 'Explore Categories',
      to:    '/tools',
      icon:  '📂',
      desc:  'Construction, farming & more',
    },
    {
      label: 'Help Center',
      to:    '/help',
  ...
  return (
    <div className="animate-fade-in w-full max-w-6xl mx-auto min-w-0">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            {/* Mode dot + label — updates immediately on mode switch */}
            <span className={`w-2 h-2 rounded-full flex-shrink-0 ${modeDot}`} />
            <span className="text-[13px] font-medium text-gray-500">{modeLabel}</span>

            {/* Separator */}
            <span className="text-gray-200 text-[13px]">·</span>
            <span className="text-gray-400 text-[13px]">{user.location || 'Nigeria'}</span>
            ...
            {/* Mode switch hint — only for dual-capability users, very subtle */}
            {canSwitchMode && (
              <>
                <span className="text-gray-200 text-[13px]">·</span>
        ...
        {/* Primary CTA — changes instantly with mode */}
            ...
            kycApproved ? (
              ...
              <Link to="/kyc"
                className="inline-flex items-center gap-2 text-sm font-medium
                           px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={chartData}
                margin={{ top: 0, right: 0, left: -20, bottom: 0 }} barSize={16}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false} />
                <XAxis dataKey="month" tick={{ fontSize: 11, fill: '#9ca3af' }}
                  axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 11, fill: '#9ca3af' }}
                ...
                <Bar dataKey="revenue" name="revenue"
                  fill="#1a5c3a" radius={[4, 4, 0, 0]} opacity={0.85} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
            ...
            {quickActions.map(({ label, to, icon, desc }) => (
              <Link key={label} to={to}
                className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50
                           active:bg-gray-100 transition-colors group">
                <div className="w-9 h-9 bg-gray-100 group-hover:bg-[#eef6f1] rounded-xl
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium text-gray-800
                                group-hover:text-[#1a5c3a] transition-colors truncate">
                    {label}
                  </p>
                  <p className="text-[11px] text-gray-400 truncate">{desc}</p>
                </div>
                <ArrowRight size={13}
                  className="text-gray-300 group-hover:text-[#3d9166] flex-shrink-0 transition-colors" />
              </Link>
            ))}
          </div>
        </SectionCard>
          ...
          {loading ? <SkeletonRows count={4} /> : (() => {
            ...
            if (!toolAlerts.length && !rows.length) return (
              <div className="text-center py-8">
                ...
                {!kycApproved && (
                  <Link to="/kyc"
                    className="text-[12px] text-[#1a5c3a] hover:underline mt-2 block">
              ...
              <div className="space-y-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-[11px] ${event.status === 'approved' ? 'text-green-600' : 'text-red-600'}`}>
                        {event.status === 'approved'
                          ...
                          : event.adminNote ? `Rejected: ${event.adminNote}` : 'Rejected by admin'}
                      </p>
                    </div>
                  </div>
                ))}
                ...
                {rows.map(booking => (
                  <div key={booking._id}
                    ...
                    <div className="w-8 h-8 bg-gray-100 group-hover:bg-gray-200 rounded-lg
      ...
      {/* ── Pending tools alert — owner mode only, only when relevant ── */}
      {isOwner && !loading && (stats?.pendingTools ?? 0) > 0 && (
        <div className="mt-4 flex items-center gap-3 bg-amber-50 border border-amber-100 rounded-2xl p-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-amber-600">
              Tools must be approved before they appear in search results.
            </p>
          </div>
          <Link to="/my-tools"
            className="text-[12px] font-semibold text-amber-700 hover:underline flex-shrink-0">
            View →
          </Link>
        </div>
      )}
